...
public class Moderator {
    ...
    public void start(Task task, DownloadManagerListenerModerator listener){
        ...
        if (taskChunks != null) {
            ...
            // notify to developer------------------------------------------------------------
            downloadManagerListener.OnDownloadStarted(task.id,task.url);
        }
    }
    ...
    public void pause(int taskID){
        ...
        if (task != null && task.state != TaskStates.PAUSED) {
            ...
            // notify to developer------------------------------------------------------------
            downloadManagerListener.OnDownloadPaused(task.id,task.url);
            
        }
    }

    public void connectionLost(int taskId,String url) {
    	downloadManagerListener.ConnectionLost(taskId,url);
    }
    ...
    public void process(int taskId, long byteRead,String url){
        ...
        if (downloadByteThreshold > THRESHOLD) {
            ...
            // notify to developer------------------------------------------------------------
            downloadManagerListener.onDownloadProcess(taskId, percent, downloadLength,url);
        }
    }
    ...
    public void rebuild(Chunk chunk){
        ...
        // notify to developer------------------------------------------------------------
        downloadManagerListener.OnDownloadFinished(task.id,task.url);
    ...
    public void reBuildIsDone(Task task, List<Chunk> taskChunks) {
        ...
        // notify to developer------------------------------------------------------------
        downloadManagerListener.OnDownloadRebuildFinished(task.id,task.url);
        ...
        // notify to developer------------------------------------------------------------
        downloadManagerListener.OnDownloadCompleted(task.id,task.url);
